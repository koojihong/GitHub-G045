...
def login():
    ...
    if request.method == "POST":
        ...
        if not email or not password:
            ...
            elif account["password"] != hash_pw(password):
...
def register():
    ...
    if request.method == "POST":
        ...
        if not all([username, email, password, confirm]):
            ...
            db = get_db()
            taken_email = db.execute("SELECT id FROM users WHERE email=?",    (email,)).fetchone()
            taken_user  = db.execute("SELECT id FROM users WHERE username=?", (username,)).fetchone()
            db.close()
            if taken_email:
        ...
        if not email or "@" not in email:
            ...
            if user:
                ...
                if send_reset_email(user["email"], reset_url):
                    return render_template("forgot_password.html", sent=reset_url, error=None)
                else:
                    error = "Could not send reset email. Please try again."
            else:
                db.close()
                # Don't reveal if email exists — show same success screen
                return render_template("forgot_password.html", sent="not_found", error=None)
